...
1. **CLI** (`src/cli.cr`, `src/coverage_reporter/cli/cmd.cr`) - Parses arguments and invokes Reporter
...
3. **Parser** (`src/coverage_reporter/parser.cr`) - Coverage file discovery and parsing:
...
6. **Config** (`src/coverage_reporter/config.cr`) - Configuration with priority: CLI args > ENV vars > YAML config
...
- `parse(filename : String) : Array(FileReport)` - Parse file and return coverage data
...
1. Create parser class in `src/coverage_reporter/parsers/my_parser.cr` inheriting `BaseParser`
...
1. Create module in `src/coverage_reporter/ci/my_ci.cr` implementing `.options` method
2. Return `nil` if ENV doesn't match CI, otherwise return `CI::Options` with extracted values
